...
public class SystemCodeFacade extends AbstractFacade<SystemCode> {
    ...
    public SystemCode findByCode(Character systemCode) {
        TypedQuery<SystemCode> q
                = em.createQuery("select s from SystemCode s where s.sCode = :code",
                        SystemCode.class);
     ...
     * @throws RuntimeException Explicitly documented here as Servlets should probably catch this
     * one just in case to avoid ugly errors to users
