...
public class TypeCodeFacade extends AbstractFacade<TypeCode> {
  ...
  @PermitAll
  public List<TypeCode> filterList(
      Character sCode, String vvCode, String grouping, int offset, int max) {
  ...
  public List<TypeCode> findBySystem(SystemCode system) {
    TypedQuery<TypeCode> q =
        em.createQuery(
            "select t from TypeCode t where t.systemCode.systemCodeId = :systemCodeId",
            TypeCode.class);
  ...
  public TypeCode findByCode(Character systemCode, String typeCode) {
    TypedQuery<TypeCode> q =
        em.createQuery(
            "select t from TypeCode t where t.systemCode.sCode = :systemCode and t.vvCode = :typeCode",
            TypeCode.class);
  ...
  public void addType(Character scode, String vvcode, String description, String grouping)
  ...
  public void editType(
      BigInteger id, Character scode, String vvcode, String description, String grouping)
      throws UserFriendlyException {
  ...
  public List<String> findGroupingList(Character sCode) {
    String sql =
        "select distinct grouping from type_code where grouping is not null order by grouping asc";

    if (sCode != null) {
      sql =
          "select distinct grouping from type_code t left join system_code s on (t.system_code_id = s.system_code_id) where s.s_code = ? and grouping is not null order by grouping asc";
    }
